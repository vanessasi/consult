...
import jpa.entities.controller.AddressJpaController;
...
public class AddressController implements Serializable {
    ...
    private AddressJpaController getJpaController() {
        if (jpaController == null) {
            jpaController = new AddressJpaController(Persistence.createEntityManagerFactory("consultPU"));
        }
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(10) {
                ...
                public int getItemsCount() {
                    return getJpaController().getAddressCount();
                }

                @Override
                public DataModel createPageDataModel() {
                    return new ListDataModel(getJpaController().findAddressEntities(getPageSize(), getPageFirstItem()));
                }
            };
        }
        return pagination;
    ...
    private void performDestroy() {
        try {
            getJpaController().destroy(current.getAddressId());
    ...
    private void updateCurrentItem() {
        int count = getJpaController().getAddressCount();
        ...
        if (selectedItemIndex >= 0) {
            current = getJpaController().findAddressEntities(1, selectedItemIndex).get(0);
        }
    }
    ...
    public static class AddressControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            return controller.getJpaController().findAddress(getKey(value));
        }
