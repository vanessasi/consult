...
import jpa.entities.controller.ClienteJpaController;
...
public class ClienteController implements Serializable {
    ...
    public Cliente getSelected() {
        if (current == null) {
            current = new Cliente();
            current.setClientePK(new jpa.entities.ClientePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    private ClienteJpaController getJpaController() {
        if (jpaController == null) {
            jpaController = new ClienteJpaController(Persistence.createEntityManagerFactory("consultPU"));
        }
    ...
    private void performDestroy() {
        try {
            getJpaController().destroy(current.getClientePK());
    ...
    public static class ClienteControllerConverter implements Converter {
        ...
        jpa.entities.ClientePK getKey(String value) {
            jpa.entities.ClientePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new jpa.entities.ClientePK();
            key.setClientName(values[0]);
            key.setClientDepartmentNumber(Short.parseShort(values[1]));
            return key;
        }

        String getStringKey(jpa.entities.ClientePK value) {
